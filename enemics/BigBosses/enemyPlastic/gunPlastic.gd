class_name GunPlastic extends Marker2D
## Represents a weapon that spawns and shoots bullets.
## The Cooldown timer controls the cooldown duration between shots.


const BULLET_VELOCITY = 850.0
const BULLET_SCENE = preload("res://enemics/BigBosses/enemyPlastic/BulletPlastic.tscn")
...
func shoot(direction: float = 1.0) -> bool:
	if not timer.is_stopped():
		return false
	for n in 8:
		var bullet := BULLET_SCENE.instantiate() as BulletPlastic
		bullet.global_position = global_position
		bullet.global_position.x += n*15
